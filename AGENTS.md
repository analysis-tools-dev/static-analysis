...
2. **Wait until the tool qualifies:** If any criterion is not met, do not submit a pull request yet. Explain that the bot closes PRs with verified criteria failures and encourage the user to submit once all requirements are met. Do not use a PR to promote a tool while it is still becoming eligible. If a criterion cannot be verified automatically, provide evidence for manual review rather than claiming it passed.
